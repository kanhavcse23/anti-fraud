...
type TransactionController struct {
	...
	coreV1 *coreV1Path.TransactionCore
	db     *gorm.DB
	logger *logrus.Logger
}

func NewTransactionController(repoV1 *repoV1Path.TransactionRepository, coreV1 *coreV1Path.TransactionCore, db *gorm.DB, logger *logrus.Logger) *TransactionController {
...
func (controller *TransactionController) CreateTransaction(w http.ResponseWriter, r *http.Request) {
	...
	var transactionReq entityHttpV1Path.CreateTransactionRequest
	...
	transaction, err := controller.coreV1.CreateTransaction(mapperV1Path.CreateTransactionPayloadMapper(&transactionReq), tx)
	if err != nil {
	...
	response := map[string]interface{}{
		"success":     true,
		"transaction": mapperV1Path.TransactionDetailsResponseMapper(transaction),
	}
